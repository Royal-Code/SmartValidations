...
- `RuleSet`: fluent DSL that accumulates `Problems` whenever a rule fails. It is a `readonly ref struct`, intended for local synchronous validation scopes.
...
- Use `Nested` for optional nested objects/collections and `NotNullNested` when null should be reported as a problem.

SmartProblems integration
- Every failing rule adds a `Problem` via `Problems.InvalidParameter(...)` with metadata like:
  - `rule` (`Rules.RuleProperty`): the rule name (e.g., `min`, `max`, `less-than`).
...
- Prefer `Min`/`Max`/`MinMax` and numeric sign rules for fixed limits; use comparison rules for comparing two values.
- Use `HttpsUrl`, `AbsoluteUrl`, `RelativeUrl`, `InPast`, `InFuture`, `Today`, `After`, `Before` and `Between` directly before falling back to `Must`.
- Avoid throwing for validation flow; return `Problems` and let callers decide.

Documentation
- See `.docs/validations.md` for the full API guide.
- See `.docs/validations.ai-rules.md` for AI-oriented usage rules.
